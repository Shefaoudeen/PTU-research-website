...
const Contact = () => {
  return (
    <>
      <div className="">
        <h1 className="text-3xl font-semibold py-10 ml-40">Contact Us</h1>
        {contact_Details.map((faculty,index) => {
          return <div key={index} className={`flex flex-wrap items-center gap-16 justify-center md:justify-start md:pl-40 py-8 bg-[${faculty.bgColor}]`}>
              <div><img src={faculty.profile} alt="profile picture" className="w-52 rounded-full" /></div>
              <div className="pr-8">
                <h1 className={`text-2xl font-semibold p-1 text-${faculty.textColor} `}>{faculty.name}</h1>
