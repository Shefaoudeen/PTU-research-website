...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <div id="menuBar" className={`${menuOpen ? "flex " : "hidden"} absolute flex-col pl-10 py-4 right-0 bg-white/50 backdrop-blur-md w-[50vw] font-light shadow-lg`}>
        {Navbar_content.map((ele, index) => {
          return (
            <NavLink to={ele.link} key={index}>
              <button id="links" className="px-4 py-3 hover:scale-110  transition-all duration-300">
                {ele.title}
              </button>
            </NavLink>
          );
